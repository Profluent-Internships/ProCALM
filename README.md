...
#to reproduce generation from selected natural language prompts
#currently only supports previously encoded text prompts that were in the ProteinDT Swissprot training set
python runner_text.py --model=text-proteinDT-swissprot --checkpoint=ba21000 --num_seqs=225 --temp=1
```

Generated sequences will be saved under `results/{model_name}/generated/` as fastas. Generation on a single GPU should take on the order of 1 minute per batch, so 900 sequences will take around 20 minutes on a single 40GB A100. For the default batch size, ba1100, ba21000, ba42000, and ba63000 refer to checkpoints trained to 1.5, 3, 6, and 9 billion tokens, respectively. 
...
```
Results will be saved under `results/{model_name}/all_summary_{checkpoint}.csv`. Analysis will fun faster on more CPUs but should take on the order of seconds to one minute per combination of unique conditioning (EC/tax). Note that you need the metadata from `raw_data/swissprot_enzyme.tsv` from [Huggingface](https://huggingface.co/jsunn-y/ProCALM) and a built BLAST reference database before this using `swissprot.fasta` and the relevant line of code from `scripts/database_setup.py`.
...
## Logging
Logging of various metrics is reported to [weights and biases](https://wandb.ai/site/). Be sure to login to your entity before training, and you can change the project name accordingly.
