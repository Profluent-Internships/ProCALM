...
def flatten(item: Any, parent_key: str = "", sep: str = "."):
    ret = dict()
    if parent_key:
        prefix = parent_key if sep and parent_key.endswith(sep) else parent_key + sep
    else:
        prefix = ""
    if isinstance(item, Mapping):
        for k, v in item.items():
            ret.update(flatten(copy.deepcopy(v), parent_key=prefix + k))
    elif isinstance(item, list):
        for i, vi in enumerate(item):
            ret.update(flatten(copy.deepcopy(vi), parent_key=prefix + str(i)))
    else:
        ret[parent_key] = copy.deepcopy(item)
    return ret
